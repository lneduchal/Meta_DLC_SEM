...
# In state 1, indicators 1, 2, and 3 load on factor 1, the remaining load on factor 2;
# in state 2, indicator 4 additionally loads on factor 1 after 15 points in time

# State 1 (factor loadings are time-invariant)
lambda_S1 <- matrix(
...
# State 2 for t = 16, ..., n_timepoints
lambda_S2_after <- matrix(
...
for (t in 2:n_timepoints){
  ...
  # Observed scores
  y_S1[t, , ] <- eta_within_S1[t, , ] %*% t(lambda_S1) + rmvnorm(
    n_persons, sigma = epsilon_S1
  )
...
for (t in 2:15){
  ...
  # Indicator scores
  y_S2[t, , ] <- eta_within_S2[t, , ] %*% t(lambda_S2_before) + rmvnorm(
    n_persons, sigma = epsilon_S2
  )
...
for (t in 16:n_timepoints){
  ...
  # Indicator scores
  y_S2[t, , ] <- eta_within_S2[t, , ] %*% t(lambda_S2_after) + rmvnorm(
    n_persons, sigma = epsilon_S2
  )
...
# Results are saved
write.csv(y, "~/Meta_DLC_SEM/data_generation/targets/y_A.csv")
write.csv(state_switch, "~/Meta_DLC_SEM/data_generation/state_switch/state_switch_A.csv")
